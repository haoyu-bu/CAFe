...
class LLaVATrainer(Trainer):
    ...
    def __init__(self, compute_language_loss, compute_contrastive_loss, emb_type, con_weight, emb_head, multimodal_input, *args, **kwargs):
        ...
        self.compute_language_loss = compute_language_loss
        ...
        rank0_print("self.compute_language_loss:", self.compute_language_loss)
        rank0_print("self.compute_contrastive_loss:", self.compute_contrastive_loss)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # from https://github.com/huggingface/transformers/blob/main/src/transformers/trainer.py
        if self.label_smoother is not None and "labels" in inputs:
            labels = inputs.pop("labels")
        else:
            labels = None
        if self.compute_contrastive_loss and self.compute_language_loss:
            outputs, text_embeds, image_embeds = model(**inputs, output_hidden_states=True, language_generation=True, compute_embedding=True, emb_type=self.emb_type, emb_head=self.emb_head, multimodal_input=self.multimodal_input)
        elif self.compute_language_loss:
            outputs = model(**inputs, output_hidden_states=True, language_generation=True, compute_embedding=False, emb_type=self.emb_type, emb_head=self.emb_head, multimodal_input=self.multimodal_input)
        elif self.compute_contrastive_loss:
            text_embeds, image_embeds = model(**inputs, output_hidden_states=True, language_generation=False, compute_embedding=True, emb_type=self.emb_type, emb_head=self.emb_head, multimodal_input=self.multimodal_input)
        
        if self.compute_contrastive_loss:
            gathered_image_features = [torch.zeros_like(image_embeds) for _ in range(dist.get_world_size())]
            gathered_text_features = [torch.zeros_like(text_embeds) for _ in range(dist.get_world_size())]
            dist.all_gather(gathered_image_features, image_embeds)
            dist.all_gather(gathered_text_features, text_embeds)
            # ensure grads for local rank when all_* features don't have a gradient
            gathered_image_features[dist.get_rank()] = image_embeds
            gathered_text_features[dist.get_rank()] = text_embeds
            all_image_features = torch.cat(gathered_image_features, dim=0)
            all_text_features = torch.cat(gathered_text_features, dim=0)

            def contrastive_loss_siglip(emb1, emb2):
                # cosine similarity as logits
                logits = (
                    torch.matmul(emb1, emb2.t().to(emb1.device)) * self.logit_scale.exp()
                    + self.logit_bias
                )
            ...
            def contrastive_loss(emb1, emb2):
                # cosine similarity as logits
                logits = (
                    torch.matmul(emb1, emb2.t().to(emb1.device)) * 20
                )
                labels = torch.arange(logits.shape[0]).long().to(logits.device)
                loss_h = torch.nn.functional.cross_entropy(logits, labels)
                loss_p = torch.nn.functional.cross_entropy(logits.T, labels)

                return (loss_h + loss_p) / 2
                    
            con_loss = contrastive_loss(all_image_features, all_text_features)
        
        if self.compute_language_loss:
            ...
            if labels is None:
                ...
                lan_loss = outputs["loss"] if isinstance(outputs, dict) else outputs[0]

        loss = self.con_weight * con_loss + lan_loss
