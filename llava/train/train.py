...
class DataArguments:
    ...
    multimodal_input: Optional[bool] = field(default=False)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    compute_language_loss: bool = field(default=False)
    compute_contrastive_loss: bool = field(default=True)
    emb_type: str = field(default='last')
    con_weight: float = 1.0
    emb_head: bool = field(default=False)
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, multimodal_input: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    for i, source in enumerate(sources):
        ...
        if multimodal_input:
            ...
            text_id = tokenizer.apply_chat_template([{"role" : "system", "content" : system_message}]) \
                                + tokenizer.apply_chat_template([{"role" : "user", "content" : ans}], add_generation_prompt=True)
            for idx, encode_id in enumerate(text_id):
                if encode_id == image_token_index:
                    text_id[idx] = IMAGE_TOKEN_INDEX            
            input_text_ids.append(text_id)
            # system message + user: <image>
            img_id = tokenizer.apply_chat_template([{"role" : "system", "content" : system_message}]) \
                                + tokenizer.apply_chat_template([{"role" : "user", "content" : qry}], add_generation_prompt=True)
        else:
            # system message + user: text
            input_text_ids.append(tokenizer.apply_chat_template([{"role" : "system", "content" : system_message}]) 
                                + tokenizer.apply_chat_template([{"role" : "user", "content" : ans + "Compress this sentence in one word:"}], add_generation_prompt=True))
            # system message + user: <image>
            img_id = tokenizer.apply_chat_template([{"role" : "system", "content" : system_message}]) \
                                + tokenizer.apply_chat_template([{"role" : "user", "content" : "<image>\nCompress this image in one word:"}], add_generation_prompt=True)
                    
        for idx, encode_id in enumerate(img_id):
            if encode_id == image_token_index:
                img_id[idx] = IMAGE_TOKEN_INDEX            
        input_image_ids.append(img_id)
    ...
    input_image_ids = torch.tensor(input_image_ids, dtype=torch.long)

    return dict(
        input_ids=input_ids,  # tensor(bs x seq_len)
        labels=targets,  # tensor(bs x seq_len)
        input_text_ids=input_text_ids, # tensor(bs x seq_len)
        input_image_ids=input_image_ids, # tensor(bs x seq_len)
    )
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, multimodal_input: bool = False) -> Dict:
    """
    ...
    if conversation_lib.default_conversation.version == "qwen":
        return preprocess_qwen(sources, tokenizer, has_image=has_image, multimodal_input=multimodal_input)
    ...
    for source in sources:
        ...
        conversation, ans = _add_speaker_and_signal(header, source)
        conversations.append(conversation)
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        ...
        self.list_data_dict = []
        self.multimodal_input = data_args.multimodal_input
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if type(image_file) is list:
                ...
                if not self.multimodal_input and len(image_file) > 1:
        ...
        data_dict = preprocess(sources, self.tokenizer, has_image=has_image, multimodal_input=self.multimodal_input)
        ...
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0], input_text_ids=data_dict["input_text_ids"][0], input_image_ids=data_dict["input_image_ids"][0])
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, input_text_ids, input_image_ids = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels", "input_text_ids", "input_image_ids"))
...
def train(attn_implementation=None):
            ...
            if "mm_language_model" in tunable_parts:
                for name, param in model.named_parameters():
                    if "vision_tower" not in name and "mm_projector" not in name and "vision_resampler" not in name:
                        param.requires_grad_(True)
            if "emb_layer" in tunable_parts:
                model.img_emb_head.requires_grad_(True)
                model.text_emb_head.requires_grad_(True)
    ...
    trainer = LLaVATrainer(model=model, tokenizer=tokenizer, args=training_args, compute_language_loss=training_args.compute_language_loss, 
                           compute_contrastive_loss=training_args.compute_contrastive_loss, emb_type=training_args.emb_type, con_weight=training_args.con_weight, 
                           emb_head=training_args.emb_head, multimodal_input=data_args.multimodal_input, **data_module)
